...
**Round 3 (deeper still)** added: schema — nutrition figures now constrained to non-negative numbers (a string/negative/null previously passed as a back-of-pack value), the year pattern tightened to 1800-2099; tests — TRUST-06 split into two surgical fixtures (each half was independently removable with the suite still green), plus a per-required-provenance-field test. Accessibility — `main:focus` contrast raised (a round-2 side-effect), disclosure chevron and nav target padding. Licensing/SEO — a standing OGL attribution footer (the FSA demo fact renders now), a per-page canonical URL from a new `site.url`, and a LICENSE-DATA Contents-rights clarification. The licensing review confirmed the share-alike posture, MIT/ODbL separation and press-quote fair dealing are all **correct**.

**Deferred to public launch (legitimately, while pre-launch behind basic auth):** the ODbL/Open Food Facts attribution footer line (add when the first OFF-derived fact renders, Phase 2 — a `{# #}` marker is in `base.njk`); `sitemap.xml` + `robots.txt`; JSON-LD / `dateModified` structured data; Open Graph tags; and updating `site.url` to the custom domain.
