...
| Phase 03a P05 | 14 | 2 tasks | 2 files |
| Phase 03a P04 | 16 | 2 tasks | 4 files |
...
- [Phase ?]: [Phase 03a]: Plan 03a-05: src/ingredient.njk paginates the ingredients object with resolve: values; INGR-02 authority block is gated on {% if ingredient.authorityPosition %} (no empty stub, distinct from regulatoryStatus, D-08), INGR-04 lists from the productsByIngredient reverse index with an honest empty state; the first record (sucralose) renders withheld by default (no fabricated passes, D-11/D-13)
- [Phase 03a]: Plan 03a-04: allergen fail-safe wording is a pure allergenLine helper (lib/allergen-copy.mjs) registered as a filter and exhaustively unit-tested; no return can emit 'does not contain', so a withheld absent claim can never read as safe (D-12)
- [Phase 03a]: Plan 03a-04: the nutrition table distinguishes three states via an 'if key in nutrition' membership test (Not recorded) vs factCell withheld (Not yet verified) vs published value, never reading .value; per-figure provenance blocks carry id=nutrition-{key} tabindex=-1 for focus management (D-06/G2)
...
Last session: 2026-07-02T07:34:44.549Z
