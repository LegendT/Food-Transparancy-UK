...
- **D-06:** Authoritative fact (what a named authority states, e.g. current GB regulatory status, the current official label): publishable only with 1 authority pass plus an independent RE-READ pass for transcription fidelity. Both passes cite the SAME authority by design, so distinct-lineage does NOT apply to authoritative facts; instead the two passes must have distinct `reviewerKind` reader-independence (human then blinded-reread, or human then ai). Two passes of the same `reviewerKind` (e.g. human then human) do NOT satisfy the re-read: `reviewerKind` is a kind, not a reviewer identity, and the model deliberately carries no reviewer-identity field, so re-read independence is honour-based and self-asserted, not mechanically enforced. The honesty model (per SPIKE-01 and SITE-02) is stated plainly: this is not two independent human reviewers, it is source-axis independence for corroborable facts and reader-axis independence for authoritative facts. (Fixes the rule asymmetry, C2; the earlier "human then a different human" example was removed post-critique because same-`reviewerKind` passes cannot be distinguished from one reviewer twice, R-13.)
...
- **D-15:** The gate derives exactly one publication state per fact, from this table (the easily-missed rule is that STALE still publishes while WRONG and DISAGREEMENT withhold):
  ...
  | `withheld-unverified` | zero recorded passes, or existence ACCESS_BLOCKED / INDETERMINATE / DOES_NOT_RESOLVE / UNCHECKED | "not yet verified - withheld" placeholder |
  | `withheld-in-review` | >=1 recorded pass but below the claim-type standard (the VRFY-03 `in-review` workflow state) | withheld placeholder |
  | `withheld-open-disagreement` | passes disagree, a measure mismatch, or confirming passes share a lineage / read different values, awaiting adjudication | withheld placeholder |
  | `withheld-wrong` | human marked the fact `wrong` | withheld placeholder (continuous, D-03) |

  Amendment (post-critique, R-02/R-04/R-07): the seven states above are the full set (the original table listed six; `withheld-in-review` was added for VRFY-03). The load-bearing rule is that every insufficient-verification condition WITHHOLDS the single fact and lets the rest of the corpus publish - it is NOT a build failure. Distinct-lineage failure, measure mismatch and cross-pass value divergence are withhold-and-route-to-human reasons derived inside `deriveVerificationState`, never entries in the build-failing error array. The prebuild gate fails ONLY on genuine internal-consistency violations (a `contested` block without a matching adjudication outcome or a non-null singular `value`; a pass `sourcesChecked` id absent from the fact's `sources`; a malformed `markedWrong`; a `derivedFrom` pointer that does not resolve to a real registry id). A cached existence verdict older than its citation TTL is treated as UNCHECKED so a since-dead link stops publishing without a rebuild of the checker (R-07).
