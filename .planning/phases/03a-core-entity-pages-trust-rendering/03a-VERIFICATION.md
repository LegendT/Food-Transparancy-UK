...
| SC5 | VRFY-11 render half: published-contested renders a visible both-sides treatment with each position's sources, distinct from withheld. VRFY-12 render half: published-stale renders "last verified {date} — review due". Both consume Phase-2-derived state; templates never re-derive | VERIFIED | Built Lucozade page: `fact__positions` with two `fact__position` items, each carrying its own resolved source link. the VRFY-12 render half (the "last verified {date} — review due" indicator, reading the verification clock not `fact.updated`) is covered by unit + behavioural tests in `test/render-state.test.js`. NOTE (superseded — see Post-audit corrections): the earlier live published-stale example on the sucralose page was a backdated proof fixture and has been re-dated to real time, so it now renders published-confirmed; no live published-stale example remains. Templates call the `factState` filter (`.eleventy.js`) which delegates to `lib/render-state.mjs`; no status re-derivation in templates. |
...
## Post-audit integrity corrections (2026-07-02)

A four-way adversarial audit of Phase 3a prompted three editor-approved corrections to the plan-06 proof set, applied after this report was first written. Claims elsewhere in this document describing a live published-stale sucralose render or an INGR-04 Lucozade link are superseded by these:

1. **Backdating removed.** The sucralose `regulatoryStatus` passes were re-dated from a backdated `2023-06-01` to the real verification date `2026-07-02`, so the fact now derives **published-confirmed**, not published-stale. The archive shows real dates only; a live published-stale example awaits a genuinely aged fact, and VRFY-12's review-due render is covered by unit + behavioural tests.
2. **Unverified cross-link removed.** The D-15 Lucozade -> sucralose `ingredients` reference was removed: Lucozade's `ingredientsText` is withheld/unresolved, so asserting the association on the sucralose page breached the per-fact gate. INGR-04 now renders its honest empty state; a live products-containing list awaits a verified product-ingredient relationship.
3. **EFSA position completed.** The `authorityPosition` value was expanded to include the 2026 opinion's fine-bakery-wares safety caveat (chlorinated-compound uncertainty at high heat), for neutrality.
