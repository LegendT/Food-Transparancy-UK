...
function isBlockedIPv4(bytes) {
  ...
  if (a === 100 && b === 100 && c === 100) return true; // 100.100.100.200 Alibaba Cloud metadata
  if (a === 192 && b === 0 && c === 0) return true; // 192.0.0.0/24 incl. Oracle Cloud metadata 192.0.0.192
...
export function isBlockedHost(hostname) {
  ...
  // Strip the FQDN root-label trailing dot: "localhost." and
  // "metadata.google.internal." resolve to the same address as the bare name,
  // but Node's URL parser preserves the dot for NAMED hosts, so an unstripped
  // "host." would slip past every name-based block below (C1).
