...
// isSoftNotFound(status, bodyText): classifyStatus is status-only, so a 200 that
// is really a not-found page would score RESOLVES. On the GET path a 200 whose
// (byte-capped) body is suspiciously tiny OR carries a common not-found marker is
// treated as a soft-404 and downgraded to INDETERMINATE upstream, never RESOLVES
// (R-22). Only fires on a 200; a real 404 is already handled by classifyStatus.
