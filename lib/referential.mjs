...
// D-15: every id in a product's plain-scalar `ingredients` array must resolve to a
// real ingredient record, else a fabricated or mistyped id would render a broken
// cross-link or a false "contained in" relationship. Mirrors checkReferences: a
// dangling product->ingredient reference fails the build.
export function checkIngredientRefs(products, ingredients) {
  const known = new Set(ingredients.map((ingredient) => ingredient.id));
  const errors = [];
  for (const product of products) {
    for (const id of product.ingredients ?? []) {
      if (!known.has(id)) {
        errors.push(`product "${product.id}": lists ingredient id "${id}" that resolves to no ingredient record`);
      }
    }
  }
  return { errors };
...
// D-15 (symmetric): every timeline event's productId must resolve to a real product
// record. This is load-bearing for the trust promise: the recipe-history join
// (timelineByProduct[product.id]) silently drops a mis-keyed event, so a product
// page would render "no recipe changes recorded yet" while a sourced change actually
// exists but is mis-linked - a page asserting a falsehood. The build must fail loud
// on a dangling timeline->product reference exactly as it does on a dangling
// ingredient reference (the archive never silently drops a provenance link).
export function checkTimelineRefs(events, products) {
  const known = new Set(products.map((product) => product.id));
  const errors = [];
  for (const event of events) {
    if (!known.has(event.productId)) {
      errors.push(`timeline event "${event.id}": productId "${event.productId}" resolves to no product record`);
    }
  }
  return { errors };
}
