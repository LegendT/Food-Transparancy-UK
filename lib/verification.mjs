...
//
// DELIBERATELY NOT wired as a build gate. A corroborable fact whose confirms share
// a lineage does NOT fail the build - it WITHHOLDS gracefully via meetsCorroborable
// (which applies the same lineageRoot logic and returns false), so the page still
// publishes its verified subset with that fact as a withheld placeholder (D-13).
// This is a tested decision: corpus-gate.test.js "R-02 ... share a lineage WITHHOLDS
// (exit 0), not a build failure". Do not add this to validate-data.mjs's error list -
// that would turn one mis-authored fact into a whole-build failure and break R-02.
// It is retained as a pure, tested helper (it also documents the lineageRoot cycle /
// dangling / tail-into-cycle edge cases) and is available for non-blocking reporting.
