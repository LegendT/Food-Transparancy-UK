...
function lineageRoot(id, sourcesById) {
  ...
  while (true) {
    ...
    if (parent === current || seen.includes(parent)) {
      return [...seen, current, parent].reduce((min, x) => (x < min ? x : min));
    }
    seen.push(current);
    current = parent;
  }
...
export function deriveVerificationState(fact, sourcesById, existenceBySourceId, today, entityType) {
  ...
  if (disputing.length > 0 || measureConflict) {
    const adj = v.adjudication;
    const clears =
      adj &&
      (adj.outcome === "confirmed" || adj.outcome === "corrected") &&
      adj.date >= latestDisagreementDate(passes, confirms, disputing, measureConflict);
    if (!clears) {
      return { state: "withheld-open-disagreement", reasons: ["passes disagree, measures mismatch, or values diverge"] };
    }
